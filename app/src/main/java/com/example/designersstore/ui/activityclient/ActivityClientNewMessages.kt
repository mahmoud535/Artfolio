...
class ActivityClientNewMessages : AppCompatActivity() {
    companion object {
        const val USER_KEY = "USER_KEY"
        private val TAG = ActivityClientNewMessages::class.java.simpleName
    }
        ...
        ref.addListenerForSingleValueEvent(object : ValueEventListener {
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                dataSnapshot.children.forEach {
                    Log.d(ActivityClientNewMessages.TAG, it.toString())
                    @Suppress("NestedLambdaShadowedImplicitParameter")
                    it.getValue(NewUser::class.java)?.let {
                        if (it.uid != FirebaseAuth.getInstance().uid) {
                            adapter.add(UserItem(it, this@ActivityClientNewMessages))
                        }
                    }
                }
                ...
                adapter.setOnItemClickListener { item, view ->
                    ...
                    val intent = Intent(view.context, ActivityChatLogClient::class.java)
